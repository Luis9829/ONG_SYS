...
namespace Capa_de_Negocios_ONG_SYS
{
    public class CN_Productos
    {
        ...
        {
            producto_cd.Insertar(NombreProducto, Convert.ToInt32(idTipoProducto),Convert.ToInt32(idProveedor),marca,Convert.ToSingle(precio),Convert.ToInt32(stock));
        }
        public void Editar(string NombreProducto, int idTipoProducto, int idProveedor, string marca, string precio, int stock,int id)
        {
            producto_cd.Actualizar(NombreProducto, Convert.ToInt32(idTipoProducto), Convert.ToInt32(idProveedor), marca,Convert.ToSingle(precio), stock,Convert.ToInt32(id));
        }
